...
| [SUPER_ADMIN.md](./SUPER_ADMIN.md) | **Super Admin operator guide** — what the `/super_admin` console controls (accounts, users, Platform Apps/`PLATFORM_TOKEN`, installation configs, Sidekiq), who can reach it, how it's protected today (separate Devise scope, throttle, MFA available behind `SUPER_ADMIN_ENFORCE_MFA`), how to create/rotate operators and enroll MFA, and the production hardening checklist |
