...
UPDATE on any pre-2026-07-04 environment (G3's operator half)."* **As of
2026-08-17 (§12.4 below), the control-plane sweep can heal the flag and
secret together automatically once its own side ships** — it no longer has to
adopt an unflagged row as-is. Until that control-plane change ships, the
manual `UPDATE` above remains the only path; once it ships, treat the manual
`UPDATE` as a **fallback** for whatever the sweep doesn't reach, not the
primary remediation.

### 12.4 Amendment — 2026-08-17: `platform_managed` is now serialized on the webhooks API — the G3 heal enabler

`_webhook.json.jbuilder` whitelisted `id`/`name`/`url`/`account_id`/
`subscriptions`/`secret`/`inbox` and silently dropped `platform_managed` — the
column existed and was filtered *on* (§12.1) but never rendered *in* the
response. That serializer-blindness is what blocked the control plane's
`verifyAllWebhooks` sweep (§12.3) from healing a pre-2026-07-04 unflagged row:
proven on 2026-08-17 when the sweep authenticated as the platform actor (so
§12.1's filter did not hide the row), listed it successfully, but had no field
to read the flag from and so could not tell "already flagged" from "still
needs healing."

`json.platform_managed webhook.platform_managed` is now added to the
serializer (additive — §8's frozen-contract rule: existing keys never change,
new keys only). Visibility is unaffected by this addition: §12.1's filter
still governs which *rows* a caller can see; this only affects which *fields*
appear on a row the caller could already see. A tenant only ever sees
`platform_managed: false` on their own rows (the only rows §12.1 lets them
list); the platform actor sees the real value on every row, including `true`
on infrastructure.

This is the enabler for §12.3's automatic heal, not the heal itself — no code
in this repo re-flags or rotates anything. The control-plane sweep reading
this field and acting on it (delete-and-recreate to also rotate the secret)
ships separately in `agentic-str`.
